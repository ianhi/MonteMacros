...
void presentationPlot5(){
  ...
  TFile * f8 = TFile::Open("ROOT/Jewel/med8_35_weights.root");
  ...
  TLegend *leg_JR = new TLegend(0.50,0.1,0.87,0.4);
  ...
  cJR->cd();
  //if(log) cJR->SetLogy();
  TH1D * JR2_m1 = (TH1D*)f1->Get("med1_Jet2_hT_HI_50");
  TH1D * JR3_m1 = (TH1D*)f1->Get("med1_Jet3_hT_HI_50");
  ...
  TH1D * JR2_m8 = (TH1D*)f8->Get("med8_Jet2_hT_HI_50");
  TH1D * JR3_m8 = (TH1D*)f8->Get("med8_Jet3_hT_HI_50");
  ...
  */
  ...
  TH1D * Jet2_hT = (TH1D*)f->Get("Jet2_hT_pp_50");
  TH1D * Jet3_hT = (TH1D*)f->Get("Jet3_hT_pp_50");
  ...
  //ratioP_hT->Draw("same");
  ratio_hT->Draw("same");
  leg_JR->SetHeader("#sqrt{s} =2.76 TeV");
  leg_JR->AddEntry(ratio1_hT,"Jewel: 0-10% R = .3");
  leg_JR->AddEntry(ratio8_hT,"Jewel: 80-90% R = .5");
  //leg_JR->AddEntry(ratioP_hT,"Pythia");
  leg_JR->AddEntry(ratio_hT,"Pythia  R = .5");
  leg_JR->Draw();
  cJR->SaveAs("PNG/PRESENTATION_JR_50.png","RECREATE");
 
  //==========================AJ============================

  TH1D*AJ_m1 = (TH1D*)f1->Get("med1_Aj_MC");
  TH1D*AJ_m8 = (TH1D*)f8->Get("med8_Aj_MC_50");
  TH1D*AJ_P  = (TH1D*)fP->Get("Pythia_Aj_MC");
  TH1D*AJ_Po = (TH1D*)f ->Get("Aj_pp_50");
  TH2D *h0_Aj = new TH2D("h0_Aj",";A_{J}",20,0,1,200,0,0.4);
  TCanvas *cAj = new TCanvas("cAj","",600,600);
  
  TLegend *leg_AJ = new TLegend(0.60,0.5,0.93,0.87);
  ...
  AJ_Po ->SetMarkerStyle(20); AJ_Po ->SetMarkerColor(kRed);

  h0_Aj->Draw();
  AJ_m1->Draw("same");
  AJ_m8->Draw("same");
  AJ_Po ->Draw("same");
  leg_AJ->SetHeader("#sqrt{s} =2.76 TeV");
  leg_AJ->AddEntry(AJ_m1,"Jewel: 0-10 R=.3");
  leg_AJ->AddEntry(AJ_m8,"Jewel: 80-90 R = .5");
  leg_AJ->AddEntry(AJ_Po ,"Pythia R = .5");
  leg_AJ->Draw();
  cAj->SaveAs("PNG/PRESENTATION_AJ_50.png","RECREATE");
}
